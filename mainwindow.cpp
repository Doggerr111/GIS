...
void MainWindow::on_actionNew_point_layer_triggered()
{
    ...
    ui->LayerTree->addTopLevelItem(item);
    project->addVectorLayer(new_layer);
...
void MainWindow::layersOrderChanged()
{
    ...
    for (int i=0; i<ui->LayerTree->topLevelItemCount(); i++)
    {
        ...
        LIPVectorLayer *layer = project->getVectorLayerByPath(path);
        if (layer!=nullptr)
        {
...
{
    //LIPVectorLayer *layer = project->getVectorLayerByPath(item->toolTip(0)); //получаем элемент по пути к файлу
    if (project->setActiveLayer(project->getVectorLayerByPath(item->toolTip(0))))
    {
        QMessageBox::information(this,"","");
    }
}
...
void MainWindow::on_pushButton_addPointFeature_clicked(bool checked)
{
    if (checked)
    {
        ...
        LIPVectorLayer *tL = project->getActiveLayer();
        if (tL!=nullptr)
...
void MainWindow::on_pushButtonTriangulationTest_clicked()
{
    ...
    while(i<pl.size()-3)
    {
        ...
        list.append(pl[i+1]);
        list.append(pl[i+2]);
        item->setPolygon(QPolygonF(list));
        ui->graphicsView->scene()->addItem(item);
        i=i+3;
    }
